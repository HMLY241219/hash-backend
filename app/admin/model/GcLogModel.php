...
abstract class GcLogModel extends BaseModel
{

    use SoftDelete;

    protected $connection = 'gc_log';
    ...
    final static function getDataSet($param, $suffix = '',$order_field1 = 'create_time')
    {
        $order_type1 = 'desc';
        $order_field2 = 'id';
        $order_type2 = 'desc';
        if (isset($param['order']) && isset($param['sort'])){
            $order_field1 = $param['order'];
            $order_type1 = $param['sort'];
            unset($param['order'],$param['sort']);
        }

        $count = self::parseWhere($param, $suffix)->count();
        $data  = self::parseWhere($param, $suffix)->order($order_field1, $order_type1)->order($order_field2,$order_type2)
    ...
    final static function getDataMulti($param)
    {
        ...
        $count = Db::connect('gc_log')->table($built . ' a')->count();
        $data  = Db::connect('gc_log')->table($built . ' a')->order($order_field1, $order_type1)->order($order_field2,$order_type2)
    ...
    private static function builtUnionSql($param = [])
    {
        ...
        foreach ($dates as $k => $vo) {
            $query = Db::connect('gc_log')->query("SHOW TABLES LIKE 't_transaction_log_{$vo}'");
            if ($query) {
