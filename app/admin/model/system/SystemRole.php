...
class SystemRole extends BaseModel
{
     ...
     * @return array
     */
    public static function getRoleLevel($level = 0)
    {
        return self::where('status',1)->where('level',$level)->column('role_name','id');
    }


    public static function getRolePid($pid = 0)
    {
        return self::where('status',1)->where('pid',$pid)->column('role_name','id');
    }


    public static function getRole($pid = 0)
    {
        return self::where('status',1)->where('pid',$pid)->column('role_name','id');
    }
    ...
    public static function getAllAuth()
    {
        ...
        $auth === null  && ($auth = self::tidyAuth(SystemMenus::all(function($query){
            $query->where('controller|action','<>','')->where('is_show','=',1)->field('id,module,controller,action,params');
        })?:[]));
        return $auth;
    }
    ...
    public static function systemPage($where){
        ...
        $model = $model->where('pid',$where['roles']);
