...
//        dd($this->sqlwhere);
        ...
        $data['data'] = Db::name('userinfo')
            ...
            a.uid,a.vip,a.coin,a.first_pay_score,
            ...
            FROM_UNIXTIME(a.login_time,'%Y-%m-%d %H:%i:%s') as login_time,e.cname,c.agent")
    ...
    public function view($uid = 0){
        ...
        $userinfo = Db::name('userinfo')
            ->alias('a')
            ->field("b.total_pay_score as daytotal_pay_score,b.total_give_score as daytotal_give_score,
            b.total_exchange as daytotal_exchange,b.cash_total_score as day_cash_total_score , b.bonus_total_score as day_bonus_total_score,
            a.uid,a.vip,d.nickname,d.status,a.coin,a.first_pay_score,d.af_status,a.withdraw_money,a.ip,
            a.total_pay_score,a.total_give_score,a.cash_total_score , a.bonus_total_score,
            a.total_exchange,a.total_cash_water_score , a.total_bonus_water_score,
            FROM_UNIXTIME(a.regist_time,'%Y-%m-%d %H:%i:%s') as regist_time,FROM_UNIXTIME(a.login_time,'%Y-%m-%d %H:%i:%s') as login_time,
            des,d.phone,d.appname,d.channel,c.puid,d.login_ip,a.bonus,a.get_bonus,d.last_pay_price,d.city,a.now_cash_score_water,a.need_cash_score_water,
            a.now_bonus_score_water,a.need_bonus_score_water")
...
//            $userinfo['cname'] = Db::name('chanel')->where('channel',$userinfo['channel'])->value('cname');
...
//        }
        ...
        $total_cash_water_score = bcadd($userinfo['total_cash_water_score'],$userinfo['total_bonus_water_score'],0);
        //总流水倍数
        $userinfo['poor_coding_volume']  = ($userinfo['total_pay_score'] + $userinfo['total_give_score']) > 0
            ? bcdiv($total_cash_water_score,bcadd($userinfo['total_give_score'],$userinfo['total_pay_score'],0),2)
            : '0';



        //用户今日获取的tpc
        $userinfo['day_bonus'] = Db::name('bonus_'.date('Ymd'))->where([['uid','=',$uid],['type','=',1],['reason','<>',3]])->sum('num');
...
//        $userinfo['loginAddress'] = $userinfo['login_ip'] ? \customlibrary\Common::get_ip_addr($userinfo['login_ip']) : '';
        ...
        //银行卡信息
        $user_withinfo_bank = Db::name('user_withinfo')->field('backname,account,ifsccode,phone,email')->where(['uid' => $uid,'type' => 1])->order('id','desc')->find();
        ...
        $userinfo['withDrawPhone'] = $user_withinfo_bank['phone'] ?? '';
        $userinfo['withDrawEmail'] = $user_withinfo_bank['email'] ?? '';

        $withdraw_log = Db::name('withdraw_log')->where([['status','in',[0,3]],['uid','=',$uid]])->column('money','id');
        $userinfo['fq_withdraw_money'] = 0;
        $userinfo['fq_withdraw_num'] = 0;
        foreach ($withdraw_log as $value){
            $userinfo['fq_withdraw_money'] += $value;
            $userinfo['fq_withdraw_num'] += 1;
        }
...
//                $v['multiple'] = bcdiv($v['water'],bcadd($v['coins'],$v['give_score'],0),0);
    ...
    public function bonus_list(){
        ...
        $date = isset($data['datetable']) && $data['datetable']? $data['datetable'] : date('Y-m-d',strtotime('-7 day')).' - '.date('Y-m-d');
        $zs_reason = config('reason.zs_reason');
        $reason = array_keys($zs_reason);
        $where = [['uid' ,'=', $data['uid']],['reason','in',$reason]];
        [$start,$end] = explode(' - ',$date);
        ...
        if($dataList)foreach ($dataList as &$v){
            $v['num'] = bcdiv((string) $v['num'],'100',2);
        }
    ...
    public function orderWithdrawIndex($uid){
        ...
        $order = Db::name('order')
            ->alias('a')
            ->leftJoin('order_active b','a.active_id = b.id')
            ->leftJoin('pay_type c','c.name = a.paytype')
            ->field("finishtime,a.price as money,1 as type,zs_bonus,zs_money,active_id,b.name,c.wake_status,a.paytype")
            ->where([['uid','=',$uid],['pay_status','=',1]])
            ->where('a.createtime','>=',strtotime($start))
            ->where('a.createtime','<',strtotime($end) + 86400)
            ->select()
            ->toArray();
        $withdraw_log = Db::name('withdraw_log')
            ->field("finishtime,really_money as money,4 as type,0 as zs_bonus,0 as zs_money,0 as active_id,100 as name,2 as wake_status,withdraw_type as paytype")
            ->where([['uid','=',$uid],['status','=',1]])
            ->where('createtime','>=',strtotime($start))
            ->where('createtime','<',strtotime($end) + 86400)
            ->select()
            ->toArray();
        ...
        $where = [['reason','in',$reson],['uid','=',$uid]];
        [$coinCount,$coinList] = Model::SubTablequery($datearray,'br_coin_',"createtime as finishtime,0 as money,reason as type,0 as zs_bonus,num as zs_money,0 as active_id,101 as name,-1 as wake_status,0 as paytype",$where,'finishtime','desc',$page,$limit);

        [$bonusCount,$bonusList] = Model::SubTablequery($datearray,'br_bonus_',"createtime as finishtime,0 as money,reason as type,num as zs_bonus,0 as zs_money,0 as active_id,101 as name,-1 as wake_status,0 as paytype",$where,'finishtime','desc',$page,$limit);

        $list = array_merge($order,$withdraw_log,$coinList,$bonusList);
        if(!$list){
            return json(['code' => 0, 'count' => 0, 'data' => []]);
        }
        ...
        $allCount =  bcadd(bcadd(bcadd($coinCount,$bonusCount,0),count($order),0),count($withdraw_log),0);
        return json(['code' => 0, 'count' => $allCount, 'data' => $newList]);
    }
    ...
    public function agent(){
        ...
        $share_strlog = Db::name('share_strlog')
            ...
            ->field('a.phone,a.package_id,a.channel,a.appname,a.nickname')->where('a.uid',$uid)->find();
