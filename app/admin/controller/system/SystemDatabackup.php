...
class SystemDatabackup extends AuthController
{
     ...
     */
    public function success2(?string $msg = null, ?string $url = null, ?array $data = [])
    {
     ...
     * @param string|null $msg
     */
    public function errors(?string $msg = null)
    {
    ...
    public function import()
    {
        ...
        if (is_numeric($time) && is_null($part) && is_null($start)) {
            ...
            if (is_array($list)) {
                ...
                $this->success2('初始化完成！', '', array('part' => 1, 'start' => 0));
            } else {
                $this->errors('备份文件可能已经损坏，请检查！');
            }
            ...
            if (false === $start) {
                ...
                if ($start[1]) {
                    ...
                    $this->success2("正在还原...#{$part}({$rate}%)", '', $data);
                } else {
                    $data['gz'] = 1;
                    $this->success2("正在还原...#{$part}", '', $data);
                }
                $this->success2("正在还原...#{$part}", '');
            }
        } else {
            $this->errors('参数错误！');
        }
