...
class Index extends AuthController
{
    ...
    public function main()
    {
        ...
        if(!$system_admin['package_id']){
            ...
            Session::get('package_id') ? true : Session::set('package_id', $packname[0]['id']);
            if ($this->adminInfo['type'] == 0) {
                ...
                $chanel = Db::name('chanel')->field('channel,cname')->where([['package_id', '=', Session::get('package_id')],['pchannel','=',0]])->where($where)->select()->toArray();
            }else{
                $chanelid = !$system_admin['channels'] ? 0 : $this->chanels($system_admin['channels'], Session::get('package_id'));
        ...
        if($chanel_cid){
            ...
            $this->sqlwhere = [['a.package_id' ,'=', $package_id]];
            $this->sqlNewWhere = ['package_id','=',$package_id];
        }
...
//            ->where($day_sum_where)
        ...
        $this->assign('package_id',Session::get('package_id'));
    ...
    public function setchanel(){
        ...
        if(!$system_admin['package_id'] && $chanel){  //选择全部的时候
            ...
            if(!in_array($chanel,$channelsarr)){
    ...
    private function chanels($channels,$package_id){
        ...
        if(Session::get('sharstatus') == 2){  //等于不包含分享数据
            $chanel_array = Db::name('chanel')->field('channel')
                ->where([['channel','in',$channels],['package_id' ,'=', $package_id]])
                ->select()
                ->toArray();
        }else{  //包含分享一级数据
            $chanel_array = Db::name('chanel')->field('channel')
