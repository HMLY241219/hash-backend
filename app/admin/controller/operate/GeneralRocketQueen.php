 ...
 *  RocketQueen普通配置
 */
class GeneralRocketQueen extends AuthController
{
    ...
    public function getlist(){
        $data =  request()->param();
        $rotary = $this->redis->hGetAll('crash4_config');
    ...
    public function edit($key){
        $other = $this->other;
        $value = $this->redis->hGet('crash4_config', $key);
        if(isset($other[$key])){
            $name = $other[$key];
        }else{
    ...
    public function save($key){
        ...
        $this->redis->hSet('crash4_config', $key, $data['value']);
