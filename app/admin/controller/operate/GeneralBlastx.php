 ...
 *  Blastx普通配置
 */
class GeneralBlastx extends AuthController
{
    ...
    public function getlist(){
        $data =  request()->param();
        $rotary = $this->redis->hGetAll('crash1_config');
    ...
    public function edit($key){
        $other = $this->other;
        $value = $this->redis->hGet('crash1_config', $key);
        if(isset($other[$key])){
            $name = $other[$key];
        }else{
    ...
    public function save($key){
        ...
        $this->redis->hSet('crash1_config', $key, $data['value']);
