 ...
 *  LuckyJet普通配置
 */
class GeneralLuckyJet extends AuthController
{
    ...
    public function getlist(){
        $data =  request()->param();
        $rotary = $this->redis->hGetAll('crash3_config');
    ...
    public function edit($key){
        $other = $this->other;
        $value = $this->redis->hGet('crash3_config', $key);
        if(isset($other[$key])){
            $name = $other[$key];
        }else{
    ...
    public function save($key){
        ...
        $this->redis->hSet('crash3_config', $key, $data['value']);
