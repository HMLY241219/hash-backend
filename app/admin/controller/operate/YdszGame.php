 ...
 *  印度骰子游戏管理列表
 */
class YdszGame extends AuthController
{
    ...
    public function getlist(){
        $data =  request()->param();
        $rotary = $this->redis->hGetAll('yd_sz_config');
        $data = [];
        foreach ($rotary as $k => $v){
            ...
            $data[] = [
                'key' => $k,
                'value' => $v,
                'name' => isset(config('game.yd_sz_config')[$k]) ? config('game.yd_sz_config')[$k] : '',
            ];
        }
    ...
    public function edit($key){
        ...
        $value = $this->redis->hGet('yd_sz_config', $key);

        $name = isset(config('game.yd_sz_config')[$key]) ? config('game.yd_sz_config')[$key] : '';
        $f = [];
    ...
    public function save($key){
        ...
        $this->redis->hSet('yd_sz_config', $key, $data['value']);
