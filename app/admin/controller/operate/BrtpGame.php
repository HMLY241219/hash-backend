 ...
 *  百人tp游戏管理列表
 */
class BrtpGame extends AuthController
{
    ...
    public function getlist(){
        $data =  request()->param();
        $rotary = $this->redis->hGetAll('bai_ran_tp_config');
        $data = [];
        foreach ($rotary as $k => $v){
            ...
            $data[] = [
                'key' => $k,
                'value' => $v,
                'name' => isset(config('game.bai_ran_tp_config')[$k]) ? config('game.bai_ran_tp_config')[$k] : '',
            ];
        }
    ...
    public function edit($key){
        ...
        $value = $this->redis->hGet('bai_ran_tp_config', $key);

        $name = isset(config('game.bai_ran_tp_config')[$key]) ? config('game.bai_ran_tp_config')[$key] : '';
        $f = [];
    ...
    public function save($key){
        ...
        $this->redis->hSet('bai_ran_tp_config', $key, $data['value']);
