 ...
 *  游戏数据列表
 */
class DailyGametop extends AuthController
{
    const VALUE = '360M';
    ...
    public function index()
    {
        ...
        $game_name_type = Db::name('slots_terrace')->where('type','<>','zy')->column('type','type');
    ...
    public function getlist(){
        ...
        $where  = Model::getWhere2($data,'date');
            ...
            $data['data'] = Db::name('daily_game_top')
        ...
        sum(pay_coin_change) as pay_coin_change,sum(no_bet_score) as no_bet_score,sum(no_coin_change) as no_coin_change")*/
                ...
                ->select()
                ->toArray();
            $data['count'] = Db::name('daily_game_top')
                ->where($where)
                ->count();
            if (!empty($data['data'])){
