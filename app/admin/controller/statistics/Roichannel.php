 ...
 *  用户管理
 */
class Roichannel extends AuthController
{
    /**
     * 用户列表
     *
...
//        $admin = $this->adminInfo;
    ...
    public function getlist(){
        ...
        $data =  request()->param();
        if (!isset($data['date']) || !isset($data['day'])){
        ...
        unset($data['date'],$data['day'], $data['type'], $data['app'], $data['network']);
        ...
        if ($type == 1) {
            $data['data'] = Db::name('statistics_roi')
                ->alias('a')
                ->field("a.*,b.bagname,c.cname")
                ->leftJoin('apppackage b', 'a.package_id = b.id')
                ->leftJoin('chanel c', 'a.channel = c.channel')
                ->where('a.time', $date)
                ->page($page, $limit)
                ->select()
                ->toArray();
            $data['count'] = Db::name('statistics_roi')
                ->alias('a')
                ->leftJoin('apppackage b', 'a.package_id = b.id')
                ->leftJoin('chanel c', 'a.channel = c.channel')
                ->where('a.time', $date)
            ...
            if ($data['data']) {
                ...
                foreach ($data['data'] as $k => &$v) {
                    ...
                    if ($val <= 0) {
                        ...
                        $v['val'] = bcdiv($val, $amount_reduction_multiple, 2);
                    }
                }
            }
            ...
            $cwhere = [];
            if ($app){
                $cwhere[] = ['a.package_id', '=', $app];
            }
            if ($network){
                $cwhere[] = ['a.channel', '=', $network];
            }
            $data['data'] = Db::name('statistics_roi')
                ->alias('a')
                ->field("a.recharge as val, b.bagname, c.cname")
                ->leftJoin('apppackage b', 'a.package_id = b.id')
                ->leftJoin('chanel c', 'a.channel = c.channel')
                ->where('a.time', $date)
                ->where($cwhere)
            ...
            if ($data['data']) {
                ...
                foreach ($data['data'] as $k => &$v) {
                    $v['val'] = bcdiv($v['val'], $amount_reduction_multiple, 2);
                }
            }
        }
