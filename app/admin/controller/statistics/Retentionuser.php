 ...
 *  用户留存
 */
class Retentionuser extends AuthController
{

    private $table = 'statistics_retained';
    ...
    public function userList(){
        ...
        $all_users = Db::name('statistics_retaineduser')
            ->where('time', $time)
            ->where($channel_where)
            ->where($NewWhere)
            ->column('uids');
...
//dd($data);
    ...
    {
        $time = strtotime($day);
        $statistics_retainedpaiduser = Db::name('statistics_retaineduser')->field('uids,package_id,channel')->where(['time' => $time])->select()->toArray();
