...
class AuthController
{
    ...
    {
        [$phone, $type, $code, $ip] = UtilService::postMore([
            ['phone', 0],
            ['type', ''],
            ['code', ''],
            ['ip', ''],
        ...
        Log::write("phone = $phone, type = $type, code = $code, ip = $ip");
        try
        {
        ...
        {
            return app('json')->make(1001, $e->getError());
        }
        ...
        if ($type == 'register')
        {
            ...
            $phoneCount    = Config::get('sms.maxPhoneCount', 10);
            $ipCount       = Config::get('sms.maxIpCount', 50);
            $maxPhoneCount = Config::get('sms.stores.' . $default . '.maxPhoneCount', $phoneCount);
            $maxIpCount    = Config::get('sms.stores.' . $default . '.maxIpCount', $ipCount);

            if (SmsRecord::where('phone', '=', $phone)->whereDay('add_time')->count() >= $maxPhoneCount)
            {
                return app('json')->make(1002, '该号码今日发送次数已达上限');
            }
            if (SmsRecord::where('add_ip', '=', $ip)->whereDay('add_time')->count() >= $maxIpCount)
            {
                return app('json')->make(1003, '该IP今日发送次数已达上限');
            }

            $time = 60;
            if (CacheService::get('code_' . $phone))
            {
                return app('json')->make(1005, $time . '秒内有效');
            }

            $data['code'] = $code = rand(100000, 999999);

            $res = ShortLetterRepositories::send(true, $phone, $data, 'VERIFICATION_CODE');
            if ($res !== true)
            {
                return app('json')->make(1004, '短信平台验证码发送失败' . $res);
            }
        ...
        {
            $real_code = CacheService::get('code_' . $phone, $code);

            if ($real_code != $code && $code != 888888) return app('json')->make(1006, '验证码不正确！');
    ...
    {
        [$account, $captcha, $password, $spread] = UtilService::postMore([
            ['account', ''],
            ['captcha', ''],
            ['password', ''],
            ['spread', 0]
    ...
    {
        [$account, $captcha, $password] = UtilService::postMore([['account', ''], ['captcha', ''], ['password', '']], $request, true);
        try {
            validate(RegisterValidates::class)->scene('register')->check(['account' => $account, 'captcha' => $captcha, 'password' => $password]);
        } catch (ValidateException $e) {
            return app('json')->fail($e->getError());
    ...
    {
        [$phone, $captcha, $spread] = UtilService::postMore([['phone', ''], ['captcha', ''], ['spread', 0]], $request, true);
    ...
    {
        [$phone, $captcha, $step] = UtilService::postMore([
            ['phone', ''],
            ['captcha', ''],
            ['step', 0]
        ], $request, true);
