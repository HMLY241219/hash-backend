...
class Tdslots extends BaseController {
    ...
    public function userAuth(Request $request){
        ...
        $data = [
            ...
            'currency' => Config::get('tdslots.currency'),
//            'balance' => ($userinfo['coin'] + $userinfo['bonus'])/100,
    ...
    public static function GetGame(){
        ...
        $url = Config::get('tdslots.api_url').'/GetGameList';
        $data = [
            'AgentId' => Config::get('tdslots.AgentId'),
        ];
    ...
    public static function Cash($ordersn,$amount,$uid){
        ...
        $url = Config::get('tdslots.api_url').'/ExchangeTransferByAgentId';
        $data = [
            'Account' => $uid,
        ...
        $res = self::senPostCurl($url,$data);
        if(!$res || $res['ErrorCode'] !== 0 || !isset($res['Data']['Status']) || $res['Data']['Status'] != 1){
            if($amount > 0) Db::name('exec_php')->insert([
                ...
                "description"=>"Td游戏充值失败回退".bcdiv($amount,100,2)."雷亚尔."
            ]);
            \customlibrary\Common::log('Tdslots充值单号'.$ordersn.'金额'.$amount.'uid'.$uid,json_encode($res),3);
    ...
    public static function transferAllOut($ordersn,$uid) {
        ...
        self::KickMember($uid);  //先给用户强制下线在转出余额
        ...
        if(!$res || $res['ErrorCode'] !== 0 || !isset($res['Data']['Status']) || $res['Data']['Status'] != 1){
            \customlibrary\Common::log('Tdslots提现单号'.$ordersn.'uid'.$uid,json_encode($res),3);
            return ['code' => 201,'msg' => '' ,'data' =>[]];
        }

        return ['code' => 200,'msg' => $res['Message'] ,'data' =>['transactionId' => $res['Data']['TransactionId'],'money' => $res['Data']['CoinBefore']]];
    ...
    public static function playerCreated($uid){
        ...
        $url = Config::get('tdslots.api_url').'/CreateMember';
        $data = [
            'Account' => $uid,
            'AgentId' => Config::get('tdslots.AgentId'),
        ];
    ...
    public static function getPlayerWallet($uid){
        ...
        $url = Config::get('tdslots.api_url').'/GetMemberInfo';
        $data = [
            'Accounts' => $uid,
            'AgentId' => Config::get('tdslots.AgentId'),
        ];
        ...
        return ['code' => 200,'msg' => $res['Message'] ,'data' =>$res['Data'][0]['Balance'] ?? 0];
    ...
    public static function getGameUrl($uid,$gameid){
        ...
        $url = Config::get('tdslots.api_url').'/singleWallet/Login';
        $data = [
            'Token' => $token,
            'GameId' => $gameid,
            'Lang' => Config::get('tdslots.language'),
            'AgentId' => Config::get('tdslots.AgentId'),
        ];
    ...
    /*public static function getGameUrl($uid,$gameid){
        ...
        $url = Config::get('tdslots.api_url').'/Login';
        $data = [
            'Account' => $uid,
            'GameId' => $gameid,
            'Lang' => Config::get('tdslots.language'),
     ...
     * @return bool|string
     */
    public static function getHistory($timepoint,$page = 1,$dataType = 'RNG') {
        $url = Config::get('tdslots.api_url').'/GetBetRecordByTime';
        $data = [
            'StartTime' => self::getUtc4tDate($timepoint),
            'EndTime' => self::getUtc4tDate($timepoint + 599),
            'Page' => $page,
            'PageLimit' => 5000,
            'AgentId' => Config::get('tdslots.AgentId'),
        ];
        ...
        $gameList = $res['Data'];
        if(count($gameList['Result']) <= 0){
            return ['timepoint' => $timepoint + 600,'gameList' => [],'pageList' =>[]];  //响应错误需要从新请求
        }

        return ['timepoint' => $timepoint + 600,'gameList' => $gameList['Result'],'pageList' =>$gameList['Pagination'] ];  //响应错误需要从新请求
    ...
    public static function KickMember($uid){
        $url = Config::get('tdslots.api_url').'/KickMember';
        $data = [
            'Account' => $uid,
            'AgentId' => Config::get('tdslots.AgentId'),
        ];
    ...
    private static function KeyG(){
        return md5(self::getDateTimeNow().Config::get('tdslots.AgentId').Config::get('tdslots.AgentKey'));
    }
