...
class SqlModel  {
   ...
   public function userDayDealWith(){
       $user_day = Db::name('user_day_'.$this->suffix)->field('uid')->where('uid',$this->updateData['uid'])->find();
       if($user_day){
           $res = Db::name('user_day_'.$this->suffix)->where('uid',$this->updateData['uid'])->update($this->updateData);
       }else{
           $res = Db::name('user_day_'.$this->suffix)->insert($this->insertData);
       }
       return $res;
   }
