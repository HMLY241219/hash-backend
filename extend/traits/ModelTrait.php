...
trait ModelTrait
{
    ...
    public static function page($model = null, $eachFn = null, $params = [], $limit = 20)
    {
        ...
        $paginate = $model === null
            ? self::paginate($limit, false, ['query' => $params])
            : $model->paginate($limit, false, ['query' => $params]);
     ...
     * @return array
     */
    public static function getModelTime($where, $model = null, $prefix = 'addtime', $data = 'data', $field = ' - ')
    {
        if ($model == null) $model = new Orders();
        ...
        switch ($where[$data]) {
            ...
            case 'quarter':
                ...
                $model = $model->where($prefix, '>', date("Y-m-d H:i:s", (strtotime($startTime) - 1)));
                $model = $model->where($prefix, '<=', date("Y-m-d H:i:s", (strtotime($endTime) + 24 * 3600)));
                break;
            default:
